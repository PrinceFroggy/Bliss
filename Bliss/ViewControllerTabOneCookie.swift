...
class ViewControllerTabOneCookie: TabViewController, WKNavigationDelegate
{
     ...
     func configBrowser()
       {
           ...
           datedate = NSDate(timeIntervalSinceNow: 3600) as Date

           date = dateFormatter.string(from: datedate!)
           ...
           print("DATE \(dateFormatter.string(from: NSDate(timeIntervalSinceNow: 3600) as Date))")
        
           print(self.abckPackage!.first!.val)
    ...
    {
        print("Loading YEEZYSUPPLY: tab one")
    ...
    func generateList()
    {
        print("STAGE 3: PRINTING COOKIES AFTER SETTING THEM & LOADING HOMEPAGE!!!!!!!!!!!!!!!!!!!!")
                ...
                for item in doc.css("article").filter({ $0.children.count >= 1 })
                {
                    ...
                    if name.range(of: wishListName) != nil
                    {
                        if item.css("p[data-auto-id='ys-product-color']").count == 1
                        {
                            ...
                            let wishListColor = self.m.YSPackage!.shoeColor.lowercased()
                                ...
                                if color.range(of: wishListColor) != nil
                                {
                                    ...
                                    //let href = item.children.first!.attr("href")
                                    //let title = item.children.first!.attr("title")
                                    //let id = item.children.first!.attr("data-auto-id")
                                
                                    self.yeezySupplyWebView!.evaluateJavaScript("$('\(item.children.first!.self)')[0].click()")
                                    
                                    //self.yeezySupplyWebView!.evaluateJavaScript("$('<a href=\"\(href!)\" title=\"\(title!)\" data-auto-id=\"\(id!)\" class=\"\">')[0].click()")
                                    
                                    //self.urlReq = URLRequest(url: URL(string: "https://www.yeezysupply.com\(href!)")!, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData, timeoutInterval: 10)
                                    //self.urlReq!.httpShouldHandleCookies = true
                        ...
                        else
                        {
                            ...
                            let href = item.parent!.children.first!.attr("href")
                            let title = item.parent!.children.first!.attr("title")
                            let id = item.parent!.children.first!.attr("data-auto-id")
    ...
    func selectSize()
    {
        print("STAGE 4: PRINTING COOKIES AT SELECT SIZE!!!!!!!!!!!!!!!!!!!!")
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Swift.Void) {
        ...
        for cookie in cookies {
            if cookie.name == "_abck"
            {
                if cookie.expiresDate != datedate{
                    ...
                    print("Cookie Name:\(newCookie?.name) - Cookie domain: \(String(describing: newCookie?.domain)) - Cookie secure: \(newCookie?.isSecure) - Cookie Value:\(newCookie?.value) - Cookie Expiry: \(String(describing: newCookie?.expiresDate)) - Cookie path: \(newCookie?.path)");
                }
                else {
