...
class ViewControllerTabThree: TabViewController, WKNavigationDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(activateKillSwitch), name: Notification.Name("tabThreeKillSwitch"), object: nil)
                ...
                self.yeezySupplyWebView!.evaluateJavaScript("$('[data-auto-id=\"yeezy-size-selection-dropdown\"]').length", completionHandler:
                    { (html: Any?, error: Error?) in
                        ...
                        if elementExists > 0
                        {
                            ...
                            {
                                NotificationCenter.default.post(name: Notification.Name("tabOneKillSwitch"), object: nil)
